...
class Track_Controller:
    def __init__(self, ctc_signals, track_signals, test=False) -> None:
        ...
        # light and switches in dictionary (key = block, value = light(0/1/2) or sw position (0,1))
    ...
    def update(self):  # Updates all values
        ...
        for i, train in self.ctc_signals.train_info.items():
            ...
            if authority[0] == "G":
                ...
                self.track_signals.commandedSpeed = self._suggested_speed[2][location - 1]
                ...
                self.track_signals.commandedSpeed = self._suggested_speed[1][location - 1]
    ...
    def get_commanded_speed(self):
        self._commanded_speed = self._suggested_speed
    ...
    def get_rail_status(self):
        for block in range(91, 241):
            if (self._broken_rail[block] == True) or (self._circuit_failure[block] == True) or (
                    self._power_failure[block] == True):
        ...
        for key in sw_1:
            if self.maintenance_switches_green == [] and self.manual_switches == 0:
                ...
                if self.maintenance_switches_green != [] and self.manual_switches == 0:
                    for m in range(len(self.maintenance_switches_green)):
                        if self.maintenance_switches_green[m][0] == key:
                            self._switch_positions[1][key - 1] = self.maintenance_switches_green[m][1]
                elif self.manual_switches == 1 and self.maintenance_switches_green == []:
                    pass
            ...
            for line in file:
                if line[0] == "s":  # initilizing switch positions
                    ...
                    if n == 1 and m == 1:  # 1 condition, 1 result
                        ...
                        if self._track_occupancy[int(block_cond[0][3:]) + 90] == eval(val_cond[0]) and \
                                self._track_occupancy[int(block_cond[1][3:]) + 90] == eval(val_cond[1]):
                            if val_res[0] == "green":
                                traffic_lights_green[int(block_res[0][3:])] = True
                                traffic_lights_yellow[int(block_res[0][3:])] = False
                                traffic_lights_red[int(block_res[0][3:])] = False
                    ...
                    if n == 1 and m == 2:
                        if eval(val_cond[0]) == False:
                            ...
                            if True not in self._track_occupancy[(90 + int(block_cond[0][3:5])):(
                                    91 + int(block_cond[0][7:9]))]:  # so if no train on any of the blocks
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                            ...
                            if True in self._track_occupancy[
                                       (90 + int(block_cond[0][3:5])):(91 + int(block_cond[0][7:9]))]:
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                        ...
                        if eval(val_cond[0]) == False:
                            ...
                            if (True not in self._track_occupancy[
                                            (90 + int(block_cond[0][3:5])):(91 + int(block_cond[0][7:9]))]) and \
                                    self._track_occupancy[int(block_cond[1][3:]) + 90] == eval(val_cond[1]):
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                            ...
                            if ((True in self._track_occupancy[
                                         (90 + int(block_cond[0][3:5])):(91 + int(block_cond[0][7:9]))]) and
                                self._track_occupancy[int(block_cond[1][3:]) + 90] == eval(val_cond[1])) and \
                                    self._track_occupancy[int(block_cond[1][3:5]) + 90] == eval(val_cond[1]):
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                    ...
                    if and_or == 0:
                        if self._track_occupancy[int(block_cond[0][3:]) + 90] == eval(val_cond[0]) and \
                                self._track_occupancy[int(block_cond[1][3:]) + 90] == eval(val_cond[1]) and \
                                self._track_occupancy[int(block_cond[2][3:]) + 90] == eval(val_cond[2]):
                            crossings[int(block_res[0][3:])] = eval(val_res[0])

                    if and_or == 1:
                        if self._track_occupancy[int(block_cond[0][3:]) + 90] == eval(val_cond[0]) or \
                                self._track_occupancy[int(block_cond[1][3:]) + 90] == eval(val_cond[1]) or \
                                self._track_occupancy[int(block_cond[2][3:]) + 90] == eval(val_cond[2]):
                            crossings[int(block_res[0][3:])] = eval(val_res[0])
                    ...
                    if line[0] == "i":  # normal if statement
                        ...
                        if n == 1 and m == 2:
                            if condition[0][10:] == "True":
                                if (True in self._track_occupancy[
                                            int(condition[0][3:5]) + 90:int(condition[0][7:9]) + 91]):
                                    if result[0][5] != "=":
                                        if result[0][7:] == "green":
                            ...
                            if (True not in self._track_occupancy[
                                            (int(condition[0][3:5]) + 90):(int(condition[0][7:9]) + 91)]) and \
                                    self._track_occupancy[int(condition[1][3:6]) + 90] == eval(condition[1][7:]):
                                if result[0][6:] == "right":
                                    switches[int(result[0][3:5])] = True
                        ...
                        if condition[0][
                            6] == "T":  # then we all evals are True in condition, so we are doing "or" operation
                            if self._track_occupancy[int(condition[0][3:5]) + 90] == eval(condition[0][6:]) or \
                                    self._track_occupancy[int(condition[1][3:5]) + 90] == eval(condition[1][6:]) or \
                                    self._track_occupancy[int(condition[2][3:5]) + 90] == eval(condition[2][6:]):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                        else:
                            if self._track_occupancy[int(condition[0][3:5]) + 90] == eval(condition[0][6:]) and \
                                    self._track_occupancy[int(condition[1][3:5]) + 90] == eval(condition[1][6:]) and \
                                    self._track_occupancy[int(condition[2][3:5]) + 90] == eval(condition[2][6:]):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                ...
                if line[0] == "s":  # First few lines in PLC text file are to initialize switches
                    ...
                    if line[0] == "i":  # normal if statement
                        ...
                        if n == 1 and m == 1:  # If we have 1 condition and 1 result
                            if condition[0][5] == "=":
                                if not (
                                not (self._track_occupancy[int(condition[0][3:5]) + 90] == eval(condition[0][6:]))):
                                    if result[0][6:] == "right":
                                        switches[int(result[0][3:5])] = True
                                ...
                                if not (
                                not (self._track_occupancy[int(condition[0][3:5]) + 90] == eval(condition[0][7:]))):
                                    if result[0][6:] == "right":
                                        switches[int(result[0][3:5])] = True
                                    elif result[0][6:] == "left":
                                        switches[int(result[0][3:5])] = False
                        ...
                        if n == 1 and m == 2:
                            if condition[0][10:] == "True":
                                if not (not ((True in self._track_occupancy[
                                                      int(condition[0][3:5]) + 90:int(condition[0][7:9]) + 91]))):
                                    if result[0][5] != "=":
                                        if result[0][7:] == "green":
                                            traffic_lights_green[int(result[0][3:6])] = True
                                ...
                                if not (not ((True not in self._track_occupancy[
                                                          int(condition[0][3:5]) + 90:int(condition[0][7:9]) + 91]))):
                                    if result[0][5] != "=":
                                        if result[0][7:] == "green":
                                            traffic_lights_green[int(result[0][3:6])] = True
                            ...
                            if not (not (True not in self._track_occupancy[(int(condition[0][3:5]) + 90):(
                                    int(condition[0][7:9]) + 91)]) or not (
                                    self._track_occupancy[int(condition[1][3:6]) + 90] == eval(condition[1][7:]))):
                                if result[0][6:] == "right":
                                    switches[int(result[0][3:5])] = True
                        ...
                        if condition[0][
                            6] == "T":  # then we all evals are True in condition, so we are doing "or" operation
                            if not (not (self._track_occupancy[int(condition[0][3:5]) + 90] == eval(
                                    condition[0][6:])) and not (
                                    self._track_occupancy[int(condition[1][3:5]) + 90] == eval(
                                    condition[1][6:])) and not (
                                    self._track_occupancy[int(condition[2][3:5]) + 90] == eval(condition[2][6:]))):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                        else:
                            if not (not (self._track_occupancy[int(condition[0][3:5]) + 90] == eval(
                                    condition[0][6:])) or not (
                                    self._track_occupancy[int(condition[1][3:5]) + 90] == eval(
                                    condition[1][6:])) or not (
                                    self._track_occupancy[int(condition[2][3:5]) + 90] == eval(condition[2][6:]))):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
            ...
            for line in file:
                if line[0] == "s":  # initilizing switch positions
                    ...
                    if n == 1 and m == 1:  # 1 condition, 1 result
                        ...
                        if self._track_occupancy[int(block_cond[0][3:]) + 14] == eval(val_cond[0]) and \
                                self._track_occupancy[int(block_cond[1][3:]) + 14] == eval(val_cond[1]):
                            if val_res[0] == "green":
                                traffic_lights_green[int(block_res[0][3:])] = True
                                traffic_lights_yellow[int(block_res[0][3:])] = False
                                traffic_lights_red[int(block_res[0][3:])] = False
                    ...
                    if n == 1 and m == 2:
                        if eval(val_cond[0]) == False:
                            ...
                            if True not in self._track_occupancy[(14 + int(block_cond[0][3:5])):(
                                    15 + int(block_cond[0][7:9]))]:  # so if no train on any of the blocks
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                            ...
                            if True in self._track_occupancy[
                                       (14 + int(block_cond[0][3:5])):(15 + int(block_cond[0][7:9]))]:
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                        ...
                        if eval(val_cond[0]) == False:
                            ...
                            if (True not in self._track_occupancy[
                                            (14 + int(block_cond[0][3:5])):(15 + int(block_cond[0][7:9]))]) and \
                                    self._track_occupancy[int(block_cond[1][3:]) + 14] == eval(val_cond[1]):
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                            ...
                            if ((True in self._track_occupancy[
                                         (14 + int(block_cond[0][3:5])):(15 + int(block_cond[0][7:9]))]) and
                                self._track_occupancy[int(block_cond[1][3:]) + 14] == eval(val_cond[1])) and \
                                    self._track_occupancy[int(block_cond[1][3:5]) + 14] == eval(val_cond[1]):
                                if val_res[0] == "green":
                                    traffic_lights_green[int(block_res[0][3:])] = True
                                    traffic_lights_yellow[int(block_res[0][3:])] = False
                                    traffic_lights_red[int(block_res[0][3:])] = False
                    ...
                    if and_or == 0:
                        if self._track_occupancy[int(block_cond[0][3:]) + 14] == eval(val_cond[0]) and \
                                self._track_occupancy[int(block_cond[1][3:]) + 14] == eval(val_cond[1]) and \
                                self._track_occupancy[int(block_cond[2][3:]) + 14] == eval(val_cond[2]):
                            crossings[int(block_res[0][3:])] = eval(val_res[0])

                    if and_or == 1:
                        if self._track_occupancy[int(block_cond[0][3:]) + 14] == eval(val_cond[0]) or \
                                self._track_occupancy[int(block_cond[1][3:]) + 14] == eval(val_cond[1]) or \
                                self._track_occupancy[int(block_cond[2][3:]) + 14] == eval(val_cond[2]):
                            crossings[int(block_res[0][3:])] = eval(val_res[0])
                ...
                if line[0] == "s":  # First few lines in PLC text file are to initialize switches
                    ...
                    if line[0] == "i":  # normal if statement
                        ...
                        if n == 1 and m == 2:
                            if condition[0][10:] == "True":
                                if (True in self._track_occupancy[
                                            int(condition[0][3:5]) + 14:int(condition[0][7:9]) + 15]):
                                    if result[0][5] != "=":
                                        if result[0][7:] == "green":
                                            traffic_lights_green[int(result[0][3:6])] = True
                                            traffic_lights_yellow[int(result[0][3:6])] = False
                            ...
                            if (True not in self._track_occupancy[
                                            (int(condition[0][3:5]) + 14):(int(condition[0][7:9]) + 15)]) and \
                                    self._track_occupancy[int(condition[1][3:5]) + 14] == eval(condition[1][6:]):
                        ...
                        if condition[0][6] == "T":  # then we all evals are True in condition, so we are doing "or" operation
                            if self._track_occupancy[int(condition[0][3:5]) + 14] == eval(condition[0][6:]) or \
                                    self._track_occupancy[int(condition[1][3:5]) + 14] == eval(condition[1][6:]) or \
                                    self._track_occupancy[int(condition[2][3:5]) + 14] == eval(condition[2][6:]):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                        else:
                            if self._track_occupancy[int(condition[0][3:5]) + 14] == eval(condition[0][6:]) and \
                                    self._track_occupancy[int(condition[1][3:5]) + 14] == eval(condition[1][6:]) and \
                                    self._track_occupancy[int(condition[2][3:5]) + 14] == eval(condition[2][6:]):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                ...
                if line[0] == "s":  # First few lines in PLC text file are to initialize switches
                    ...
                    if line[0] == "i":  # normal if statement
                        ...
                        if n == 1 and m == 1:  # If we have 1 condition and 1 result
                            if condition[0][5] == "=":
                                if not (
                                not (self._track_occupancy[int(condition[0][3:5]) + 14] == eval(condition[0][6:]))):
                                    if result[0][6:] == "right":
                                        switches[int(result[0][3:5])] = True
                                ...
                                if not (
                                not (self._track_occupancy[int(condition[0][3:5]) + 14] == eval(condition[0][7:]))):
                                    if result[0][6:] == "right":
                                        switches[int(result[0][3:5])] = True
                                    elif result[0][6:] == "left":
                                        switches[int(result[0][3:5])] = False
                        ...
                        if n == 1 and m == 2:
                            ...
                            if not (not (True not in self._track_occupancy[(int(condition[0][3:5]) + 14):(
                                    int(condition[0][7:9]) + 15)]) or not (
                                    self._track_occupancy[int(condition[1][3:5]) + 14] == eval(condition[1][6:]))):
                        ...
                        if condition[0][
                            6] == "T":  # then we all evals are True in condition, so we are doing "or" operation
                            if not (not (self._track_occupancy[int(condition[0][3:5]) + 14] == eval(
                                    condition[0][6:])) and not (
                                    self._track_occupancy[int(condition[1][3:5]) + 14] == eval(
                                    condition[1][6:])) and not (
                                    self._track_occupancy[int(condition[2][3:5]) + 14] == eval(condition[2][6:]))):
                                crossings[int(result[0][3:5])] = eval(result[0][6:])
                        else:
