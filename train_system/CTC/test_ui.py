...
class testUI(QMainWindow):
    ...
    def stationUpdate(self):
        id = int(self.comboBox_8.currentText()) - 1
        value = self.spinBox.value()
        self.CTC.editStation(id, value)

    def trackUpdate(self):
        id = int(self.comboBox_2.currentText()) - 1
        value = self.checkBox.isChecked()
        self.CTC.editTrack(id, value)
    ...
    def trainUpdate(self, d: int):
        ...
        if(d == 2):
            t = self.comboBox_7.currentText()
            location = self.comboBox_6.currentText()
            passenger = self.numPassengers.value()

        if (t == "New Train"):
            self.CTC.addTrain(destination, 0)
            self.updateCombo()
            self.trainTableUpdate()

        else:
            if(d == 1):
                self.CTC.editTrain(int(t)-1, destination, str(0))
            if(d == 2):
                self.CTC.editTrain2(int(t)-1, location, passenger)
            self.trainTableUpdate()

    def updateCombo(self):
        t = self.CTC.getTrainList()
        for i in range(self.comboBox_7.count()-1, len(t)):
            self.comboBox_7.addItem(str(len(t)))
            self.comboBox_3.addItem(str(len(t)))

    def trackTableUpdate(self):
        index = []
        trackList = self.CTC.getTrackList()
        count = trackList.count(False)
    ...
    def trainTableUpdate(self):
        ...
        self.tableWidget.setRowCount(len(trainList))
        for i in range(len(trainList)):
            self.tableWidget.setItem(i, 0, QtWidgets.QTableWidgetItem(str(trainList[i].train_id)))
            self.tableWidget.setItem(i, 1, QtWidgets.QTableWidgetItem(str(trainList[i].location)))
            self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(trainList[i].destination))
            self.tableWidget.setItem(i, 3, QtWidgets.QTableWidgetItem(str(trainList[i].authority)))
            self.tableWidget.setItem(i, 4, QtWidgets.QTableWidgetItem(str(trainList[i].suggested_speed)))
