...
class programmer_UI(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        self.label_way = QtWidgets.QLabel(self.groupBox)
        self.label_way.setObjectName("")
        self.verticalLayout_15.addWidget(self.label_way)
        self.label_auto = QtWidgets.QLabel(self.groupBox)
        self.label_auto.setObjectName("")
        self.verticalLayout_15.addWidget(self.label_auto)
    ...
    def arduino_com(self):
        # Clears inputs and outputs to prevent filling memory
        port.flushInput()
        port.flushOutput()

        self.line_ = 1  # Red line
        self.tempBlock = port.readline()  # Reads the serial port

        if len(self.tempBlock) < 4:  # If arduino is just sending current block number
            # Set indexes

            print(self.tempBlock)  # Print statement for testing purposes
            # Error checking in case of random issue, exits the function back to update
            try:
                int(self.tempBlock)
            except ValueError:
                return

            self.label_auto.setText("Automatic Mode")  # Sets label to automatic mode

            # Checks if block is in 1st wayside
            if int(self.tempBlock) < 38:
                self.wayside = 0
                self.usedBlock = int(self.tempBlock)
                self.label_way.setText("Wayside 1")
            # Checks if block is in second wayside
            else:
                self.wayside = 1
                self.usedBlock = int(self.tempBlock) - 38
                self.label_way.setText("Wayside 2")

            # Sets indexes
            self.label_1.setText(f'Block {self.wayside * 38 + self.usedBlock + 1}')
            self.block = self.wayside * 38 + self.usedBlock
            self.b = self.wayside * 38 + self.usedBlock + 15


            # Send automatic block data
            # 12 size string
            ...
            # 9-11 = Authority
            # Adds block number
            if(int(self.tempBlock)<10):
                self.sendData = '0' + str(int(self.tempBlock))
            else:
                self.sendData = str(int(self.tempBlock))

            # self.sendData += str(int(self._track_controller.get_switch_positions()[self.line_][self.block]))

            # Adds traffic light
            if(self._track_controller.get_traffic_lights()[0][self.line_][self.block] == True):
                self.sendData = self.sendData + '0'

            elif(self._track_controller.get_traffic_lights()[1][self.line_][self.block] == True):
                self.sendData = self.sendData + '1'
            else:
                self.sendData = self.sendData + '2'


            # self.sendData = self.sendData + str(int(self._track_controller.get_crossings()[self.line_][self.block]))

            # Adds commanded speed
            if(self._track_controller.get_commanded_speed()[self.line_][self.block] < 10):
                self.sendData = self.sendData + '0' + str(int(self._track_controller.get_commanded_speed()[self.line_][self.block]))
            else:
                self.sendData = self.sendData + str(int(self._track_controller.get_commanded_speed()[self.line_][self.block]))
            # self.sendData = self.sendData + "111"

            # Adds track occupancy
            self.sendData = self.sendData + str(int(self._track_controller.get_track_occupancy()[self.b]))

            # Adds track status
            if(self._track_controller.get_broken_rail()[self.b]==1 or self._track_controller.get_circuit_failure()[self.b]==1 or self._track_controller.get_power_failure()[self.b] == 1):
                self.sendData = self.sendData + '0'
            else:
                self.sendData = self.sendData + '1'

            # Adds switch position
            if self._track_controller.get_switch_positions()[self.line_][self.block] == True or self._track_controller.get_switch_positions()[self.line_][self.block] == False:
            ...
            # Adds crossings
            if self._track_controller.get_crossings()[self.line_][self.block] == True or self._track_controller.get_crossings()[self.line_][self.block] == False:

                self.sendData += str(int(self._track_controller.get_crossings()[self.line_][self.block]))
            else:
                self.sendData += '2'

            # Adds authority
            self.sendData += self._track_controller.get_authority()[self.line_][self.block] +"000"
            # Adds buffer to signify end
            self.sendData += ','

            # Sends data to the serial port
            port.write(bytes(self.sendData, 'utf-8'))

            print(self.sendData)  # Print for error checking



        else:
            # Manual setting serial data
            # 5 size string
            # 0 - 1 = Current Block number
            # 2 = Switch position
            # 3 = Light state
            ...
            print(self.tempBlock)  # Print for error checking

            self.label_auto.setText("Manual Mode")  # Sets label to manual mode

            # Sets wayside 1
            if ((int(self.tempBlock[0])-48)*10) + (int(self.tempBlock[1])-48) < 38:
                self.wayside = 0
                self.usedBlock = ((int(self.tempBlock[0])-48)*10) + (int(self.tempBlock[1])-48)
                self.label_way.setText("Wayside 1")

            # Sets wayside 2
            else:
                self.wayside = 1
                self.usedBlock = ((int(self.tempBlock[0])-48)*10) + (int(self.tempBlock[1])-48) - 38
                self.label_way.setText("Wayside 2")
            ...
            print(self.block)  # Print for error checking

            # Sets switchs if switch is present on track
            if self.block == 15 or self.block == 51:
                if (int(self.tempBlock[2])-48) == 0:
                    self.switches_set_left(True)
                else:
                    self.switches_set_right(True)

            # Sets Lights if light is present
            if self.block == 0 or self.block == 1 or self.block == 64 or self.block == 65:
                if (int(self.tempBlock[3])-48) == 0:
                    self.lights_green(True)
                elif (int(self.tempBlock[3])-48) == 1:
                    self.lights_yellow(True)
                else:
                    self.lights_red(True)

            # Sets crossings if crossings are present
            if self.block == 46:
                if (int(self.tempBlock[4])-48) == 0:
    ...
    def _update(self):
        # Runs arduino communication function
        self.arduino_com()

        # Sets all values from signals
